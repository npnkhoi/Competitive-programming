#include <bits/stdc++.h>
using namespace std;
typedef pair<int, int> ii;
typedef long long ll;
typedef vector<int> vi;
#define fi first
#define se second
#define mp make_pair
#define pb push_back
#define REP(i,n) for (int i = 0; i < n; i++)
#define FOR(i,a,b) for (int i = a; i <= b; i++)
#define FORD(i,a,b) for (int i = a; i >= b; i--)
#define task "SENDINFOR"
const int nTest = 100;

ll Rand(ll l, ll r) {
    return l + (
    1LL * rand() * (RAND_MAX + 1) * (RAND_MAX + 1) * (RAND_MAX + 1) +
    1LL * rand() * (RAND_MAX + 1) * (RAND_MAX + 1) +
    1LL * rand() * (RAND_MAX + 1) +
    1LL * rand()) % (r - l + 1);
}
set<pair<int, int> > data;

const int A = 5;

bool inRec(int x, int y) {
    return (1 <= x && x <= 3 && 1 <= y && y <= 3);
}

void test() {
    data.clear();
    ofstream inp(task".inp");
    int n = Rand(1, 10);
    inp << n << "\n";
    for (int i = 1; i <= n; ++ i) {
        int x, y;
        do {
            x = Rand(-A, A), y = Rand(-A, A);
        } while (data.find(make_pair(x, y)) != data.end() || inRec(x, y));
        inp << x << " " << y << "\n";
        data.insert(make_pair(x, y));
        // inp << Rand(0, 5) << " " << Rand(0, 5) << "\n";
    }
    inp << "4\n";
    inp << "1 1\n";
    inp << "3 1\n";
    inp << "3 2\n";
    inp << "2 3\n";
    inp.close();
}
int main() {
    //freopen(task".inp", "r", stdin);
    //freopen(task".out", "w", stdout);
    srand(time(NULL));
    FOR(love, 1, nTest) {
        printf("*TEST NUMBER %d\n", love);
        test();

        double timer1 = clock();
        system(task".exe");
        timer1 = (clock() - timer1) / CLOCKS_PER_SEC;
        double timer2 = clock();
        system(task"_sol.exe");
        timer2 = (clock() - timer2) / CLOCKS_PER_SEC;
        printf("Time: %lf %lf\n", timer1, timer2);

        if (system("fc "task".out "task".ok")) {
        // if (system("fc "task".out "task".out")) {
            printf("WRONG ANSWER\n");
            return 0;
        }
        printf("CORRECT\n");
    }
}
