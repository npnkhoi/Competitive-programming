...
void sub3() {
	//f(0, i, k, p1, p2) -> f(0, j, l, p1', p2') 
	for (int t = 0; t <= 1; ++ t) {
		for (int i = 0; i <= n; ++ i) {
			for (int k = 0; k <= MAX_LEN; ++ k) {
				for (int ml = 0; ml <= MAX_LEN; ++ ml) {
					for (int mr = 0; mr <= MAX_LEN; ++ mr) {
						f[t][i][k][ml][mr] = oo;
					}
				}
			}
	...
	f[0][0][0][0][0] = 0;
	heap.push(Pack(0, 0, 0, 0, 0, 0));
	while (!heap.empty()) {
		Pack t = heap.top();
		heap.pop();

		if (f[t.side][t.i][t.rem][t.ml][t.mr] != t.dist) continue;

		for (int i = 1; i <= n; ++ i) {
			if (!match(0, i, k, j)) continue; // check palin
			l = min(k, (int) s[j].size());
			newML = ;
			if (newML + t.mr >= len) {
				res = min(res, t.dist + (int) s[j].size());
				continue;
			}
			newMR = ;
			if (newMR)
			...
			newI = 
			newRem = 
			if (f[newSide][newID][newRem][newML][newMR] > f[t.side][t.id][t.rem][t.ml][t.mr] + (int) s[j].size()) {
				f[newSide][newID][newRem][newML][newMR] = f[t.side][t.id][t.rem][t.ml][t.mr] + (int) s[j].size();
				heap.push(Pack(f[newSide][newID][newK][newML][newMR], newSide, newID, newRem, newML, newmR));
			}
		}
	}
}	
